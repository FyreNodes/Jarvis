...
export const run: CommandRun =  async (client, message, args) => {
    ...
    switch (args[0]) {
        case 'view':
            ...
            let viewEmbed = new MessageEmbed({ author: { name: `Infraction's | ${member.user.tag}`, iconURL: member.user.avatarURL() }, color: '#1AB6DC', description: viewInf.map((data) => {return `**ID:** ${data.infID} **|** Moderator: ${message.guild.members.cache.get(data.details.moderator).user.tag} **|** Reason: ${data.details.reason} ${data.details.duration ? `**|** Duration: ${data.details.duration}` : ``}`}).join('\n'), thumbnail: { url: member.user.avatarURL() }, footer: { text: 'Moderation • Jarvis', iconURL: client.user.avatarURL() }, timestamp: Date.now() });
